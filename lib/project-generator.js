...
import { getConfig, getLanguage, TEMPLATE_PATH, TEMPLATE_REMOTE_FILE, mkdirp } from './util';
import { TemplateRepo } from './updater';
import archive from 'ls-archive';
import path from 'path';
import { promisify } from 'util';
import { createWriteStream, mkdir, readFile } from 'fs';
...
export default class ProjectGenerator {
  ...
  generate() {
    ...
    .then(() => {
      this.view.hide();
      atom.open({pathsToOpen: [options.path], newWindow: true});
    })
    .catch(err => {
      if (err) {
        atom.notifications.addError('An error occured while trying to generate a project', { icon: 'zap', detail: err, dismissable: true });
        console.err(err);
      }
    });
  }
  ...
  return mkdirp(destPath, PROJECT_ROOT_DIR())
        ...
        .then(() => {
          return new Promise((resolve, reject) => {
            exec('gradle wrapper', { cwd: destPath }, (err, stdout, stderr) => {
              if (stderr) {
                reject(stderr);
              } else if (err) {
                atom.notifications.addError('Gradle threw an error while trying to generate wrappers', { icon: 'zap', detail: stderr, dismissable: true});
                console.err(stderr);
                reject(stderr);
              } else {
                resolve(stdout);
