...
const GRADLERIO_VER_URI = 'github.com/wpilibsuite/GradleRIO/releases/latest';
...
const getsURI = (uri, ...ps) => 'https://' + path.join(uri, ...ps);

const getGradleRioUri = ver => {
    return {
        java: `github.com/wpilibsuite/GradleRIO/releases/download/${ver}/java.zip`,
        cpp: `github.com/wpilibsuite/GradleRIO/releases/download/${ver}/cpp.zip`
    };
};
...
export const GradleRioRepo = {
    niceName: 'GradleRIO',
    dir(...paths) {
        return path.join(GRADLERIO_PATH, ...paths);
    },
    getLatestVersion(lang)  {
        return new Promise((resolve, reject) => {
            gets(getsURI(GRADLERIO_VER_URI), response => {
                if (response.statusCode > 300 && response.statusCode < 400 && response.headers.location) {
                    let newUrl = response.headers.location.split('/');
                    let ver = newUrl[newUrl.length - 1];
                    resolve(ver);
                } else {
                    reject(response);
                }
    ...
    update(lang) {
        let destFile = this.dir(GRADLERIO_REMOTE_FILE(lang));
        return this.getLatestVersion(lang)
               .then(ver => {
                   return ifExists(destFile)
                          .then(ver => {
                              if (ver == getConfig('gradlerioVersion')) {
                                  throw 'Up to date';
                              } else {
                                  setConfig('gradlerioVersion', ver);
                              }
                              return ver;
                          })
                          .catch(err => {
            ...
            .then(getGradleRioUri)
            .then(uris => downloadsToFile(getsURI(uris[lang]), destFile))
            .catch(() => true);
    }
};

function downloadsToFile(url, filePath) {
    return downloads(url).then(data => saveToFile(data, filePath));
}
...
function downloads(url) {
    return new Promise((resolve, reject) => {
        gets(url, response => {
            if (response.statusCode > 300 && response.statusCode < 400 && response.headers.location) {
                downloads(response.headers.location).then(resolve);
            } else {
                resolve(response);
            }
            })
