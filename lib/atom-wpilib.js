...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'wpilib:update-gradlerio': () => this.updateGradleRio(),
  ...
  startSmartDashboard() {
    ...
    fs.access(buildFile, err => {
      ...
      exec(buildFile + "-q smartdashboard",
        (err, _, stderr) => {
          if (err) {
            atom.notifications.addError("An error was encountered while trying to start SmartDashboard", {
              stack: err.toString(),
              icon: 'zap'
            });
          } else if (stderr) {
            atom.notifications.addError("Gradle threw an error while trying to start SmartDashboard", {
              stack: err.toString(),
              icon: 'zap'
            });
          }
  ...
  startShuffleboard() {
    ...
    fs.access(buildFile, err => {
      ...
      exec(buildFile + "-q shuffleboard",
        (err, _, stderr) => {
          if (err) {
            atom.notifications.addError("An error was encountered while trying to start Shuffleboard", {
              stack: err.toString(),
              icon: 'zap'
            });
          } else if (stderr) {
            atom.notifications.addError("Gradle threw an error while trying to start Shuffleboard", {
              stack: err.toString(),
              icon: 'zap'
            });
          }
  ...
  updateGradleRio() {
    this.updatedThisSession.gradleRio = true;
    return this.updateGeneric(GradleRioRepo);
  },
  ...
  generateProject() {
    ...
    if (getConfig('autoUpdate') === true && (this.updatedThisSession.templates && this.updatedThisSession.gradleRio) === false) {
      this.updateGradleRio().then(() => this.updateTemplates()).then(() => generate());
    } else {
      generate();
    }
  },
  ...
  viewExample() {
    ...
    if (getConfig('autoUpdate') === true && (this.updatedThisSession.examples && this.updatedThisSession.gradleRio) === false) {
      this.updateGradleRio().then(() => this.updateExamples()).then(() => this.exampleView.show());
    } else {
